...
async function Top3() {
    ...
    if (data.length < 3) {
        throw new BannerError("No se tiene suficiente información de artistas para crear un banner!");
    }
    if (!user.name) {
        throw new BannerError("Se requiere que el usuario tenga un nombre!");
    }
    if (!user.image) {
        throw new BannerError("Se requiere que el usuario tenga una imágen de perfil!");
    }
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (impar) {
            ...
            context.fillText(artistaData.name, canvas.width - size - textMeasurament.width - gap - textGap, sY);
            drawImage(canvas.width - gap - size, sY - size / 2, image, size, size, shadowSettings);
        }
